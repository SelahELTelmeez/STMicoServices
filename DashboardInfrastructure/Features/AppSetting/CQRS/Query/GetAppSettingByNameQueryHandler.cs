...
public class GetAppSettingByNameQueryHandler : IRequestHandler<GetAppSettingByNameQuery, ICommitResult<AppSettingResponse>>
{
    ...
    {
        return ResultType.Ok.GetValueCommitResult(await _dbContext.Set<DomainEntities.AppSetting>().Where(a => a.Name.Equals(request.Name)).ProjectToType<AppSettingResponse>().SingleOrDefaultAsync(cancellationToken));
    }
}